...
// FIXED: Import from your localized FRONTEND client utility file instead of the backend folder
...
function Editor() {
  ...
  const socketRef = useRef(null); // Reference layer holding the live connection context
  ...
  // Initialize clients state list as an empty array to be populated dynamically by the server mapping
  ...
  useEffect(() => { 
    const initConnection = async () => { 
      // Fire up frontend socket.io client tunnel link
      socketRef.current = await initSocket();

      // Error handler alert logs
      ...
      // FIXED: Swapped event string payload key from 'join' to match backend 'join-room' hook
      ...
      // LISTEN 2: Fired when an alternative peer enters the channel matrix context
      socketRef.current.on("user-joined", ({ userName, clients }) => {
        setClients(clients);
        setTerminalLogs((p) => [...p, `[System]: Peer connection established: ${userName} entered workspace.`]);
      });
      ...
      socketRef.current.on("user-left", ({ userName, socketId }) => {
        setTerminalLogs((p) => [...p, `[System]: Peer connection terminated: ${userName} left workspace.`]);
    ...
    // Disconnect connection loop when component unmounts to prevent pipeline memory leaks
    return () => {
  ...
  const handleCodeChange = (e) => {
    ...
    // Emit live typing change stream up to the server socket matrix layer
    if (socketRef.current) {
  ...
  const runCode = () => {
    ...
    setTimeout(() => {
      if (code.includes("console.log")) {
        setTerminalLogs((prev) => [...prev, `Initializing local environment...`, `System Check: 100% Operational`, `[Success]: Process exited cleanly.`]);
      } else {
